...
#'                 'year'
#' @param plot_mode What package will be used to produce the plot, options:
#'                  ggplot, bokeh, highcharter
#'
#' @return A list containt p = ggplot object, d = the data frame used in the plot
#' @export
plot_generic_compare <- function(dat, params, compare, disciminent, cum_plot=FALSE, plot_mode="ggplot") {
  ...
  if (plot_mode == "bokeh") {
    p <- figure(title = params$title) %>%
      ly_bar(compare, outcome, data = dat, color = disc, hover = TRUE)
  }
